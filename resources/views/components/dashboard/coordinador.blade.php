<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white p-6 rounded-3xl border border-rose-200 shadow-sm relative overflow-hidden">
            ...
            <div class="flex justify-between items-start mb-4 relative z-10">
                ...
                <span class="px-2.5 py-1 bg-rose-100 text-rose-600 text-xs font-black rounded-lg">{{ $docentesSinMarcar->count() }} Alerta(s)</span>
            </div>
            <div class="space-y-3 relative z-10 max-h-[220px] overflow-y-auto">
                @forelse($docentesSinMarcar as $docente)
                    <div class="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-100">
                        <span class="text-xs font-bold text-slate-700">{{ $docente->nombre_completo }}</span>
                        <span class="text-[10px] font-black text-rose-500 uppercase tracking-widest">Sin Registrar</span>
                    </div>
                @empty
                    <p class="text-xs font-medium text-slate-500 py-3 text-center">Todos los docentes registraron su asistencia hoy.</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl border border-amber-200 shadow-sm relative overflow-hidden">
            ...
            <div class="flex justify-between items-start mb-4 relative z-10">
                ...
                <span class="px-2.5 py-1 bg-amber-100 text-amber-600 text-xs font-black rounded-lg">{{ $solicitudesPendientes->count() }} Petición(es)</span>
            </div>
            <div class="space-y-3 relative z-10 max-h-[220px] overflow-y-auto">
                @forelse($solicitudesPendientes as $solicitud)
                    <div class="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-100">
                        <span class="text-xs font-bold text-slate-700">
                            {{ $solicitud->docente?->usuario?->nombre_completo ?? 'Docente' }} — {{ $solicitud->nota?->aulaAsignaturaDocente?->asignatura?->nombre ?? 'Asignatura' }}
                        </span>
                        <a href="{{ route('academico.notas.solicitudes.index', ['estado' => 'Pendiente']) }}" class="text-[10px] font-black text-amber-600 hover:text-amber-700 uppercase tracking-widest bg-amber-50 px-3 py-1.5 rounded-lg border border-amber-200 transition-colors">Evaluar</a>
                    </div>
                @empty
                    <p class="text-xs font-medium text-slate-500 py-3 text-center">No hay solicitudes de desbloqueo pendientes.</p>
                @endforelse
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
        ...
        <!-- 2. Visor de Horarios -->
        <a href="{{ route('academico.visor.index') }}" class="text-left group bg-white p-5 rounded-3xl border border-slate-200 shadow-sm hover:border-[#e6ac27] hover:shadow-md transition-all flex flex-col h-full cursor-pointer w-full">
            <div class="w-12 h-12 rounded-2xl bg-slate-50 border border-slate-100 text-[#e6ac27] flex items-center justify-center group-hover:bg-[#e6ac27] group-hover:text-white transition-colors mb-4">
