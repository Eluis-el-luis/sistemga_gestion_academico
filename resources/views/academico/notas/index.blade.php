<x-app-layout>
    ...
    <div class="py-10 bg-[#FFFDF5] min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if($modoSupervision)
                <div class="bg-white p-6 rounded-3xl border border-stone-200 shadow-sm mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                @forelse($asignaciones as $asignacion)
                    <a href="{{ route('academico.notas.actividades.index', $asignacion->id) }}" class="group bg-white rounded-3xl border border-stone-200 p-6 shadow-sm hover:shadow-md hover:border-[#e6ac27] transition-all relative overflow-hidden flex flex-col justify-between">
                        ...
                        <div class="relative z-10">
                            ...
                            @if($modoSupervision)
                                <p class="text-[10px] font-bold text-stone-400 mt-2 uppercase tracking-widest flex items-center gap-1.5">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                    {{ $asignacion->docente->usuario->nombre_completo ?? 'SIN DOCENTE' }}
                                </p>
                            @endif
                        </div>
                ...
                @empty
                    <div class="col-span-full bg-white p-12 text-center rounded-3xl border border-stone-200 shadow-sm flex flex-col items-center justify-center">
                        ...
                        <p class="text-sm font-medium text-stone-400 mt-1">Esta aula no tiene asignaturas vinculadas o el docente seleccionado no posee carga horaria actual.</p>
                    </div>
                @endforelse
            </div>
        </div>
