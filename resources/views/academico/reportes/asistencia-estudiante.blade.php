<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-black text-2xl text-[#3d2c1d] leading-tight">Estadísticas de Asistencia por Estudiante</h2>
    ...
    <div class="pb-12 pt-6 max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="bg-white rounded-3xl shadow-sm border border-slate-200 p-6">
            <form method="GET" action="{{ route('academico.reportes.asistencia-estudiante') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                ...
                <a href="{{ route('academico.reportes.asistencia-estudiante') }}" class="self-end text-center text-sm font-bold text-slate-400 hover:text-rose-600 py-2">Limpiar</a>
            </form>
        </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($filas as $fila)
                            <tr class="hover:bg-slate-50">
                                <td class="px-6 py-4 font-bold">{{ $fila['cup'] }}</td>
                                <td class="px-6 py-4 font-bold text-[#3d2c1d]">{{ $fila['alumno'] }}</td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span class="px-2.5 py-1 rounded-lg text-[10px] font-black {{ $fila['porcentaje'] >= 90 ? 'bg-emerald-100 text-emerald-700' : ($fila['porcentaje'] >= 75 ? 'bg-amber-100 text-amber-700' : 'bg-rose-100 text-rose-700') }}">
                                        {{ $fila['porcentaje'] }}%
                                    </span>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="7" class="px-6 py-12 text-center text-stone-500 font-bold">Sin datos de asistencia.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
