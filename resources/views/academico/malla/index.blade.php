<x-app-layout>
    ...
    <div class="pb-12 pt-6" x-data="{ modalidadFiltro: localStorage.getItem('mallaTab') || 'Todos', modalClonar: false }">
        <div class="max-w-[90rem] mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <!-- SECCIÓN 1: FORMULARIO SUPERIOR EXPANDIDO -->
            <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="bg-[#FFFDF5] px-8 py-5 border-b border-[#e6ac27]/20 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                    ...
                    <!-- BOTÓN CLONAR MALLA REUBICADO AQUÍ -->
                    <button @click="modalClonar = true" class="px-5 py-2.5 bg-slate-800 hover:bg-slate-700 text-white text-xs font-black uppercase tracking-widest rounded-xl shadow-md transition-all flex items-center gap-2 whitespace-nowrap">
            ...
            <!-- SECCIÓN 2: VISUALIZADOR Y GESTIÓN AVANZADA -->
            <div>
                <!-- PESTAÑAS DE FILTRADO (CON MEMORIA LOCALSTORAGE) -->
                ...
                <div class="flex flex-wrap gap-2 mb-6 bg-white p-2 rounded-2xl border border-slate-200 w-fit shadow-sm">
                    <button @click="modalidadFiltro = 'Todos'; localStorage.setItem('mallaTab', 'Todos')" :class="modalidadFiltro === 'Todos' ? 'bg-[#e6ac27] text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'" class="px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Todos</button>
                    @foreach($modalidades as $mod)
                        <button @click="modalidadFiltro = '{{ $mod }}'; localStorage.setItem('mallaTab', '{{ $mod }}')" :class="modalidadFiltro === '{{ $mod }}' ? 'bg-[#e6ac27] text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'" class="px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">{{ $mod }}</button>
                    @endforeach
                </div>

                <!-- CUADRÍCULA DE ACORDEONES (AHORA A 3 COLUMNAS MÁXIMO) -->
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($grados as $grado)
                        ...
                        <div x-data="{ abierto: false }" 
                             x-show="modalidadFiltro === 'Todos' || modalidadFiltro === '{{ $grado->modalidad->nombre ?? '' }}'" x-transition 
                             class="bg-white rounded-3xl shadow-sm border {{ $estaLleno ? 'border-rose-200' : 'border-slate-200' }} overflow-hidden flex flex-col hover:shadow-md transition-all h-fit">
                            
                            <!-- Cabecera del Acordeón -->
                            <div class="px-6 py-5 cursor-pointer select-none group" :class="abierto ? 'bg-[#FFFDF5] border-b border-[#e6ac27]/20' : 'bg-white'" @click="abierto = !abierto">
                                <div class="flex justify-between items-start mb-3">
                                    <div>
                                        <h4 class="font-black text-[#3d2c1d] text-lg group-hover:text-[#e6ac27] transition-colors">{{ $grado->nombre }}</h4>
                                        <div class="flex items-center gap-2 mt-1">
                                            <span class="bg-slate-100 border border-slate-200 text-slate-600 px-2 py-0.5 rounded-lg text-[9px] uppercase tracking-widest font-black">{{ $grado->modalidad->nombre ?? 'N/A' }}</span>
                                            <span class="text-[10px] font-bold text-slate-400">• {{ $grado->mallaCurricular->count() }} Materias</span>
                                        </div>
                                    </div>
                                    <div class="text-right flex items-center justify-end gap-3">
                                        <div class="flex flex-col items-end">
                                            <span class="text-xl font-black leading-none {{ $estaLleno ? 'text-rose-600' : 'text-[#e6ac27]' }}">{{ $horasUsadas }}</span>
                                            <form action="{{ route('academico.malla.grado.horas', $grado->id) }}" method="POST" class="inline-block form-editar-horas" @click.stop>
                                                @csrf @method('PUT')
                                                <input type="hidden" name="horas_maximas_semanales" class="input-horas-oculto">
                                                <button type="button" class="text-[10px] font-bold text-slate-400 hover:text-[#e6ac27] transition-colors flex items-center gap-0.5 mt-0.5 btn-editar-horas" data-grado="{{ $grado->nombre }}" data-horas="{{ $limiteHoras }}" title="Editar límite">
                                                    / {{ $limiteHoras }}h
                                                </button>
                                            </form>
                                        </div>
                                        <!-- Ícono Desplegable -->
                                        <div class="w-8 h-8 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 transition-transform duration-300" :class="abierto ? 'rotate-180 bg-[#e6ac27]/10 text-[#e6ac27] border-[#e6ac27]/20' : ''">
                            ...
                            <!-- Contenido Desplegable (Materias) -->
                            <div x-show="abierto" x-collapse x-transition class="p-6 bg-white border-t border-slate-50" style="display: none;">
                                @if($grado->mallaCurricular->count() > 0)
                                    <ul class="space-y-3">
                                        @foreach($grado->mallaCurricular as $item)
                                            <li class="flex justify-between items-center text-sm border-b border-slate-50 pb-3 last:border-0 last:pb-0 group">
                                                <div class="flex flex-col">
                                                    <span class="font-bold text-slate-700 group-hover:text-[#e6ac27] transition-colors">{{ $item->asignatura->nombre }}</span>
                                                    <span class="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-0.5">{{ number_format($item->horas_semanales_sugeridas ?? 0, 0) }} horas / sem</span>
                                                </div>
                                                ...
                                                <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <form action="{{ route('academico.malla.item.update', $item->id) }}" method="POST" class="form-editar-item">
                                                        ...
                                                        <button type="button" class="text-slate-300 hover:text-amber-500 hover:bg-amber-50 p-1.5 rounded-lg transition-colors btn-editar-item" data-nombre="{{ $item->asignatura->nombre }}" data-horas="{{ $item->horas_semanales_sugeridas }}" title="Editar horas">
                                                    ...
                                                    <form action="{{ route('academico.malla.destroy', $item->id) }}" method="POST" class="alerta-eliminar">
                                                        @csrf @method('DELETE')
                                                        <button type="submit" class="text-slate-300 hover:text-rose-500 hover:bg-rose-50 p-1.5 rounded-lg transition-colors" title="Quitar de la malla">
                                ...
                                @else
                                    <div class="flex flex-col items-center justify-center text-center py-6 bg-slate-50/50 rounded-2xl">
                                        <svg class="w-8 h-8 text-slate-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                        <p class="text-xs text-slate-400 font-bold uppercase tracking-widest">Plantilla Vacía</p>
                                    </div>
                                @endif
                            </div>
                        </div>
