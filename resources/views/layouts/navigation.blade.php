<nav x-data="{ open: false }" class="bg-white border-b border-amber-200/80 shadow-sm sticky top-0 z-40">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-6 sm:-my-px sm:flex">
                    ...
                    @endcan
                    @can('malla.ver')
                        <x-nav-link :href="'#'" :active="false" class="hover:text-amber-800">Académico</x-nav-link>
                    @endcan
                    @can('reportes.ver')
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3.5 py-2 border border-amber-200 text-sm leading-4 font-bold rounded-lg text-amber-900 bg-amber-50/60 hover:bg-amber-100 hover:text-amber-950 focus:outline-none transition ease-in-out duration-150 shadow-sm gap-2">
                            <span>{{ Auth::user()->primer_nombre ?? Auth::user()->name ?? Auth::user()->username ?? 'Usuario' }}</span>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-amber-50/90 border-t border-amber-200">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @can('alumnos.ver')
                <x-responsive-nav-link :href="route('academico.alumnos.index')">Alumnos General</x-responsive-nav-link>
            ...
            @can('notas.ver')
                <x-responsive-nav-link :href="'#'">Calificaciones</x-responsive-nav-link>
            @endcan

            @can('asistencia.ver')
                <x-responsive-nav-link :href="'#'">Asistencia</x-responsive-nav-link>
            @endcan
        </div>

        <div class="pt-4 pb-1 border-t border-amber-200 px-4">
            <div class="font-bold text-base text-amber-900">{{ Auth::user()->primer_nombre ?? Auth::user()->name ?? Auth::user()->username ?? 'Usuario' }}</div>
